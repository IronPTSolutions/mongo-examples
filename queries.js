...
db.restaurants.updateMany({cuisine: "American"}, {$set: {cuisine: "American"}}).pretty()
//Update Morris Park Bake Shop address street to Calle falsa 123.
db.restaurants.updateMany({name: "Morris Park Bake Shop"}, {$set: {"address.street": "Calle Falsa 123"} }  ).pretty()
//Delete all the restaurants with address zipcode 10466.
db.restaurants.deleteMany({"address.zipcode": "10466"}).pretty()
...
db.companies.find({ category_code:"enterprise", founded_year: 2005},{name:1, category_code:1, founded_year:1 }).pretty()
...
db.companies.find({founded_month: {$ne: 6}},{founded_month:1, name:1}).skip(50).pretty()
...
db.companies.find({founded_day: 1, $nor:[{number_of_employees: 50}, {category_code: "web"}]}, {founded_day:1, name:1}).limit(5).pretty()
