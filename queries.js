...
db.restaurants.find({ 'grades.score': { $gt: 90 } }, { name: 1, 'grades.score': 1, _id: 0 })
// Find the restaurants that do not prepare any Bakery cuisine and with a grade score equal or bigger than 70.
db.restaurants.find({ cuisine: { $ne: 'Bakery' }, 'grades.score': { $gte: 70 } }, { name: 1, cuisine: 1, 'grades.score': 1, _id: 0 })
...
db.restaurants.find({ cuisine: { $ne: 'Chinese' }, 'grades.grade': 'A', borough: { $ne: 'Manhattan' } }, { name: 1, 'grades.grade': 1, borough: 1, _id: 0 }).pretty()
...
db.restaurants.updateOne({ name: 'Morris Park Bake Shop' }, { $set: { 'address.street': 'Calle falsa', 'address.building': '123' } })
...
db.companies.find({ $or: [{ founded_year: 2000, number_of_employees: { $gte: 20 } }] }, { founded_year: 1, number_of_employees: 1, _id: 0 }).sort({ number_of_employees: -1 })
...
db.companies.find({ category_code: { $in: ['web', 'social'] } }, { name: 1, category_code: 1, _id: 0 }).limit(20)
...
db.companies.find({ founded_month: { $ne: 6 } }, { name: 1, founded_month: 1, _id: 0 }).limit(50)
// Find all the companies that have 50 employees, but do not correspond to the 'web' category_code.
db.companies.find({ number_of_employees: 50, category_code: { $ne: 'web' } }, { number_of_employees: 1, category_code: 1, _id: 0 })
...
db.companies.find({ founded_month: 1, number_of_employees: { $lt: 50 }, category_code: { $ne: 'web' } }, { name: 1, founded_day: 1, _id: 0 }).limit(5)
...
db.companies.find({ $and: [{ 'acquisition.acquired_year': 2014 }, { 'acquisition.acquired_month': 1 }] }, { name: 1, _id: 0 })