...
db.restaurants.find({}, {name: 1, restaurant_id: 1, borough: 1, 'address.zipcode': 1})
...
db.companies.find({ $or: [ {founded_year: 2000}, {number_of_employees: 20} ] }).sort( { amount: -1 } )
...
db.companies.find({'founded_month': { $ne: 6}}, {'founded_month':1, 'name':1}).skip(50)
...
db.companies.find({ $or: [{"category_code":{$ne:"web"}}, {"number_of_employees":{$ne:50}}], "founded_day" : 1}, {"name": 1, "founded_day": 1}).limit(5)
...
db.companies.find({"acquisition.acquired_month" : 1, "acquisition.acquired_year": 2014}, {"name": 1, "acquisition": 1})
