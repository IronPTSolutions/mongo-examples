...
//Find employee with whose name is Steve
...
db.restaurants.find( {}, { _id: 1, name: 1, borough: 1, "adress.zipcode": 1 })
...
////Find the restauran ts which have achieved a score bigger than 90.

db.restaurants.find( { $and: [ { "borough": "Brooklyn" }, { "cuisine": "Steak" } ] } )

////Find the restauran ts that do not prepare any Bakery cuisine and with a grade score equal or bigger than 70.

db.restaurants.find( { $and: [ { "cuisine": { $ne: "Bakery" } }, { "grades.score": {$gt: 70} } ] } )

////Find the restauran ts which do not prepare any Chinese cuisine and have achieved a grade point A which do not belong to the borough Manhattan.
...
db.companies.find( { "name": {$in: ["Facebook"]}  })
...
db.companies.find( { $and: [ { "founded_day": 1 }, { $nor: [ { "number_of_employees": 50 }, { "category_code": "web" } ] } ] }, { _id: 0, founded_day: 1, name: 1 } ).limit(5)

//Find all the companies which the price_amount of the acquisition was 40.000.000. Sort them by name.

db.companies.find({"acquisition.price_amount": 40000000}).sort("name")

//Find all the companies that have been acquired on January of 2014. Retrieve only the acquisition and name fields.

db.companies.find({$and: [{"acquired_month": 1},{"acquired_year": 2014}]}, {_id: 0, acquisition: 1, name: 1})