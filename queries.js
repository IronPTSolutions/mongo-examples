...
db.restaurants.find({},{fields:1, restaurant_id :1, name :1, borough:1, cuisine:1,_id:0})
...
db.restaurants.find({$and:[{cuisine:"Bakery"},{"grade.score":{$gte:70}}]})
...
db.restaurants.find({$and: [{cuisine:!"Chinese"},{borough:!"Manhattan"},{"grades.grade":"A"}]})
...
db.restaurants.deleteMany({ "adress.zipcode": "10466"})
...
// Find all the companies that have been founded on the 1st of the month, 
...
db.companies.find({$and:[{category_code:{$ne:"web"}},{number_of_employees:{$ne:50}},{founded_day:1}]},{category_code:1,name:1,_id:0}).limit(5)
