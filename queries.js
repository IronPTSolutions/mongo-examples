...
db.restaurants.find({$and: [{cuisine: {$not:{$eq:"Bakery"}}},{"grades.score":{$gte:70}}]}).pretty()
...
db.restaurants.find({$and: [{cuisine: {$not:{$eq:"Chinese"}}},{"grades.grade":{$eq:"A"}},{borough: {$not:{$eq:"Manhattan"}}}]}).pretty()
...
db.companies.find({$and: [{category_code: {$not:{$eq:"web"}}},{category_code: {$not:{$eq:"social"}}}]},{name:1, category_code:1}).limit(20).pretty()
...
db.companies.find({founded_month:{$not:{$eq:6}}},{name:1, founded_month:1}).skip(50).pretty()
// Find all the companies that have 50 employees, but do not correspond to the 'web' category_code.
db.companies.find({$and: [{category_code: {$not:{$eq:"web"}}},{number_of_employees: {$eq:50}}]},{name:1, category_code:1,number_of_employees:1}).pretty()
...
db.companies.find({$and: [{founded_day:{$eq:1}},{number_of_employees: {$ne:50}},{category_code: {$not:{$eq:"web"}}}]},{name:1, founded_day:1}).limit(5).pretty()
...
db.companies.find({$and:[{"acquisition.acquired_year":{$eq:2014}},{"acquisition.acquired_month":{$eq:1}}]},{name:1,acquisition:1}).pretty()