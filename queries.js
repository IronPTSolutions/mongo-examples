...
db.restaurnts.find({"borough":"Bronx"})
...
db.companies.find({ $and : [ { "category.code" : {$ne: "web"}} , { "category.code" : {$ne: "social"}} ] } , { "name": 1, "category_code": 1, "_id": 0 }).limit(20)
...
db.companies.find({$and : [{"number_of_employees": {$eq: 50}} , { "category.code" : {$ne: "web"}} ]}).pretty()
