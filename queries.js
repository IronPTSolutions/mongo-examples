...
db.companies.find({ founded_month: { $ne: 6 } }, { _id: 0, founded_month: 1, name: 1 }).skip(50)
...
db.companies.find({ founded_day: 1, number_of_employees: { $ne: 50 }, category_code: { $ne: "web" } }, { _id: 0, name: 1, founded_day: 1, }).limit(5)
// Find all the companies which the price_amount of the acquisition was 40.000.000.Sort them by name.
db.companies.find({ "acquisition.price_amount": 40000000 }, { _id: 0, name: 1, number_of_employees: 1 }).sort({ name: 1 })
// Find all the companies that have been acquired on January of 2014. Retrieve only the acquisition and name fields.
db.companies.find({ "acquisition.acquired_year": 2014, "acquisition.acquired_month": 1 }, { _id: 0, name: 1, acquisition: 1 })