...
db.employees.updateOne({ name: 'Bob' }, { $set: { privileges: "true" } });
...
db.restaurants.find({"cuisine": {$ne: "Bakery"},"grades.score": { $gt: 90 }})
...
db.restaurants.updateOne({ "address.street": 'Morris Park Bake Shop' }, { $set: { "Morris Park Bake Shop": "Calle falsa 123" } });
...
db.companies.find({"founded_month":1, $nor: [{"number_of_employees":50},{"category_code":"web"}]},{"founded_day": 1, "name": 1, "_id": 0}).limit(5)
