...
db.employees.updateOne({name: "Willy"},{$set: {"phone.personal": "93-123-45-67"}}).pretty()
//Change Bob’s privilege to normal user.
db.employees.updateOne({name: "Bob"},{$set: {privileges: "user"}}).pretty()
...
db.restaurants.updateMany({name: {$eq: "Morris Park Ave"}},{$set: {"address.street": "Calle falsa 123"}})
...
db.companies.find({category_code: {$eq: "web"}},{name: 1}).pretty()
//Find all the companies named "Twitter", and retrieve only their name, category_code and founded_year fields.
db.companies.find({name: {$eq: "Twitter"}},{name: 1, category_code: 1, founded_year: 1}).pretty()
...
db.companies.find({category_code: {$eq: "enterprise"}, founded_year: {$eq: 2005}}, {name: 1, category_code: 1, founded_year: 1}).pretty()
...
db.companies.find({ $or: [{founded_year: {$eq: 2000}}, {number_of_employees: {$eq: 20}}]}).sort({number_of_employees: 1}).pretty()
...
db.companies.find({founded_year: {$ne: "June"}}, {founded_month: 1, name: 1}).skip(50).pretty()
...
db.companies.find({$and: [{ "founded_day": 1}, {"number_of_employees": {$ne: 50}}, {"category_code": {$ne: "web"}}]}, {"_id": 0, "founded_day": 1,"name": 1}).limit(5).pretty()
...
db.companies.find({$and: [{"acquisition.acquired_year": 2014}, {"acquisition.acquired_month": 1}]}, {acquisition: 1, name: 1}).pretty()
