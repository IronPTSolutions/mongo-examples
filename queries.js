...
db.restaurants.find({ borough: { $eq: "Bronx" , _id: 0 }}).pretty() 
...
db.companies.find({$or : [{"category_code": {$ne: "web"}}, {"category_code": {$ne: "social"}}]}, {name: 1, category_code: 1, _id:0} ).limit(20).pretty()
...
db.companies.find({"founded_day": {$eq: 1}, "number_of_employees": {$ne: 50}, "category_code": {$ne: "web"}}, {"founded_day": 1, "name": 1, _id: 0}).limit(5).pretty()
